...
| 目录 | 用途 | 操作规则 |
|:----|:----|:----|
| `00-系统/` | Ground Truth 层 | 身份/规则/信任层级，**永不衰减**，P0 优先级 |
...
| `07-地图/` | 主题地图 + 结晶摘要 | 理解路线图 + 每次工作会话的结晶 |
| `08-草稿/` | 输出草稿 | 文章、推文、脚本 |
| `99-归档/` | 归档 | 暂时不用但不想删除 |
...
| `status` | ✅ | 页面状态：draft / active / needs-review / superseded / archived |
| `trust_score` | ⚠️ | 数值 0.0~1.0，高=可信。规则见 RULEBOOK.md 规则6 |
| `confidence` | ⚠️ | 文本置信度：high（30天内确认）/ medium（30-90天）/ low（90天+）|
| `superseded_by` | ⚠️ | 被哪篇新笔记取代（status=superseded时必填）|
| `last_confirmed` | ⚠️ | 上次确认真实的日期，留空=updated |
| `contradiction_with` | ⚠️ | 与之矛盾的笔记 `[[笔记名]]`，双方都要加 |
| `priority` | ⚠️ | P0~P3，P0=不可覆盖（仅SOUL/RULEBOOK/GROUND_TRUTHS用）|
...
| **needs-review** | 可能过时 | trust_score ≤ 0.5，等待人工复核 | 黄色虚线 |
| **superseded** | 已被取代 | 保留引用关系，指向 superseded_by | 灰色 |
