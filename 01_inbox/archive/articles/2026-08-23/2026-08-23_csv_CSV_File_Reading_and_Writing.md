...
Instructs `writer` objects to only quote those fields which contain special characters such as _delimiter_ , _quotechar_ , `'\r'`, `'\n'` or any of the characters in _lineterminator_. If _doublequote_ is [`False`](constants.html#False "False") and _escapechar_ is set, the _quotechar_ is escaped instead of causing the field to be quoted.
...
Changed in version 3.10: Previously the _escapechar_ itself was not escaped, which lost it on reading.
